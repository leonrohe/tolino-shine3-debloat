...
`mass_storage` and `rndis`, and ships no `MtpService`. Mass storage is the
...
Sharing the volume takes `/storage/sdcard1` away from Android, and KOReader
...
- Before relaunching KOReader the helper waits for the volume to be genuinely
  usable, checking both `!isVolumeShared()` and the mount state rather than
  sleeping for a fixed time. An earlier blind 8 s wait produced
  `Invalid mkdirs path` crashes.
...
- `KILL_BACKGROUND_PROCESSES`: stops KOReader before its volume is taken away.
...
`build.sh` generates a debug keystore on first run. **Rebuilding on another
machine produces a differently-signed APK.** That does not matter for installing
into `/system/app` (root replaces the file directly), but it does mean
`adb install -r` across machines will hit a signature mismatch. Uninstall first,
or use the `/system/app` path.
