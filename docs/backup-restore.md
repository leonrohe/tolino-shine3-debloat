...
Requires root, which is why the order is: **root in RAM → back up → modify**.
...
Needs only the bootloader, so **it works even when Android will not boot.**
...
Why the extra step: the bootloader caps a single download at **352 MiB**, and a
full `/system` image is 384 MiB — it is rejected instantly with
`FAILED (remote: '')`. The script shrinks the filesystem to fit while keeping
~80 MB free. See `findings.md` §3a.
...
For the byte-exact original image (which cannot be flashed — it is over the cap).
From a rooted session:
...
Path 1 avoids all of this, which is exactly why it exists.

## Restore path 3: back to stock

The official firmware at `/share/orig_update.zip` on the device restores
everything — debloat, framework patch and helper included. That is what a factory
recovery is for. Apply it through stock recovery, **not** by putting it in the
storage root (`update.zip` there would make the reader self-update on the next
restart and silently undo every change).
