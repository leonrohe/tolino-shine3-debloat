...
factory reset leaves the device debloated: it removes only apps that happened to
be installed in `/data` (KOReader). The store app cannot come back that way,
since the wipe never touches the partition it lives on.
...
> from there on the next restart, which undoes your work at a time you did not
> choose.
...
`upgrade.sh` is the vendor's own script, and the part that could touch raw
eMMC. It contains `dump_uboot()`, `dump_hw_config()`, `dump_logo()` and
`dump_waveform()`, all of which `dd` straight to `/dev/block/mmcblk0`, outside
any partition. On a Tolino Shine 3 (`E60K00`), none of them run:
...
The other raw writes are gated on files that do not exist in this payload, so
their `dd` cannot run either:
...
of `$UPDATE_ZIP_PATH`, and with no arguments that variable is resolved only from
...
Since the updater-script invokes it with no arguments, and none of those paths
exist when the payload sits at `/emergency/orig_update.zip`, the unzip produces
nothing, `/cache/upgrade` stays empty, and every `dd` fails harmlessly.

Net result on a Shine 3: only `/system` (p5), boot (p1) and recovery (p2) are
written. Every one of those is recoverable over fastboot. `/share` (p10) is not
formatted, so on-device backups and the payload itself survive.
...
If it fails partway, `/system` is left formatted and Android will not boot.
...
carries the `adb` flag, lives in `/data/property`. So the USB gadget comes back
without adb, and you can see it change:
...
Type the code into the reader's search field and submit the search:
...
(These change between major versions, so guides for 15.x do not work on 16.x.
Source: [clickomania.ch](https://blog.clickomania.ch/2025/06/20/apk-installation-auf-dem-tolino/),
confirmed against a Shine 3 on 16.2.0.)

You page through the menu with the on-screen buttons; **page 3 installs APKs**
from the reader's storage root. Turning the debug menu on is what restores the
`adb` flag in `persist.sys.usb.config`, and you can watch it happen on the host
side: the gadget product ID changes back:
